...
function preflight_checks() {
    ...
    command -v bsdtar > /dev/null 2>&1 || {
        echo "ERROR: Please install bsdtar (on Debian, that's in libarchive-tools)"
        exit 1
    }
    command -v xorriso > /dev/null 2>&1 || {
        echo "ERROR: Please install xorriso"
        exit 1
    }
}
...
orig_iso="${1:?ERROR: Pass the original Debian 13 netinst .iso as argument}"
